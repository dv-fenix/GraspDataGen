...
class GraspingDisplay:
    ...
    def load_grasp_file(self):
        ...
        # Always visualize the PREGRASP state, never the final post-sim transform.
        pose_key_pos = "pregrasp_position"
        pose_key_orn = "pregrasp_orientation"
        ...
        # Fallbacks in case the file lacks explicit pregrasp entries.
        if cspace_key not in first_grasp:
            cspace_key = "cspace_position"
        if bite_key not in first_grasp:
            bite_key = "bite_point"
        ...
        for i in range(num_grasps):
            ...
            pos_src = grasp.get(pose_key_pos, grasp["position"])
            orn_src = grasp.get(pose_key_orn, grasp["orientation"])

            position = np.array(pos_src, dtype=np.float32)
            orientation_xyz = np.array(orn_src["xyz"], dtype=np.float32)
            orientation_w = np.float32(orn_src["w"])

            grasps[i] = np.concatenate([position, orientation_xyz, [orientation_w]])
            bite_points[i] = np.array(grasp.get(bite_key, grasp["bite_point"]), dtype=np.float32)

            cspace_src = grasp.get(cspace_key, grasp["cspace_position"])
            for j, cspace_joint_name in enumerate(cspace_joint_names):
                cspace_positions[i][j] = cspace_src[cspace_joint_name]
